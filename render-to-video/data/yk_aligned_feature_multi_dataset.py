...
class YkAlignedFeatureMultiDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        imglistA = os.path.join(opt.dataroot, 'list/%s/%s.txt' % (opt.phase+'A', opt.dataname))
        imglistB = os.path.join(opt.dataroot, 'list/%s/%s.txt' % (opt.phase+'B', opt.dataname))
    ...
    def __getitem__(self, index):
        ...
        for i in range(1,self.Nw):
            ...
            path1 = AB_path.replace(frame,'frame%d' % (max(0, frameno-i)))
        ...
        if self.opt.isTrain or self.opt.test_use_gt:
            ...
            ss = AB_path2.split('/')
            B_feat_path = os.path.join('/'.join(ss[:-2]), 'arcface', ss[-2], ss[-1][:-4] + '.npy')
            idx = _iframe(ss[-1][:-4])
            if not os.path.exists(B_feat_path):
                org_path = B_feat_path
                for delta in [-1, 1, -2, 2]:
                    B_feat_path = os.path.join('/'.join(ss[:-2]), 'arcface', ss[-2], f'frame{idx-delta}.npy')
                    if os.path.exists(B_feat_path):
                        break
                assert os.path.exists(B_feat_path)
                print("Warning: failed to find {}, use adjacent frame {}".format(org_path, B_feat_path))
            B_feat = np.load(B_feat_path)
            item['B_feat'] = B_feat
